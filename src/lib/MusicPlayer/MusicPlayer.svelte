<script>
	...
	let toggleButton = $state(playButton);
	let time = $state(0);
	...
	$effect(() => {
		({ audio, title, cover } = Array.isArray(playlist) ? playlist[currentSongIndex] : playlist);
	});
...
<div class="audioplayer">
	<div>
		<div class={playerClassName}>
			...
			<img src={coverBg} class="cover-bg" alt="" width="200" height="200" />
		</div>
	</div>
	<div class="panel">
		...
		<div class="controls">
			<button class="icon" onclick={() => (time -= 15)}
				><img src={rewindButton} alt="rewind" width="28" height="28" /></button
			>
			<button class="icon" onclick={() => (time += 15)}
				><img src={forwardButton} alt="forward" width="28" height="28" /></button
			>
